...
export const submitTask = asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, message: 'Work submitted successfully', data: task });
});
...
export const assignTask = asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    data: { task, assignedJunior: { id: bid.freelancerId, name: bid.freelancerName, email: bid.freelancerEmail } }
  });
});
...
export const getTaskStats = asyncHandler(async (req, res) => {
  ...
  const totalBudget = await Task.aggregate([{ $group: { _id: null, total: { $sum: '$budget' } } }]);
